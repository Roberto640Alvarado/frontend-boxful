...
const EMPTY_PAQUETE: Paquete = { largo: 0, alto: 0, ancho: 0, peso: 0, contenido: "" };
...
useEffect(() => {
  ...
  const handleAdd = () => {
    if (!draft.contenido.trim() || !draft.peso || !draft.largo || !draft.alto || !draft.ancho) {
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8 w-full">
        ...
        {/* Contenedor input */}
        <div className="rounded-xl p-4 mb-4" style={{ background: "#f9fafb", border: "1.5px solid #e5e7eb" }}>
          <div className="flex flex-wrap items-end gap-3">
            <div className="hidden sm:flex items-center justify-center w-10 h-10 rounded-lg flex-shrink-0 self-end mb-0.5">
              <BoxIcon size={22} />
            </div>
            <DimInput label="Largo" name="largo" value={draft.largo} onChange={handleDraftChange} />
            <DimInput label="Alto" name="alto" value={draft.alto} onChange={handleDraftChange} />
            <DimInput label="Ancho" name="ancho" value={draft.ancho} onChange={handleDraftChange} />
            <div className="flex flex-col gap-1" style={{ minWidth: "110px" }}>
              <label className="text-xs text-gray-900">Peso en libras</label>
              <SmallInput name="peso" type="number" min="0" step="0.1" placeholder="3 libras"
                value={draft.peso || ""} onChange={handleDraftChange} />
            </div>
            <div className="flex flex-col gap-1 flex-1" style={{ minWidth: "150px" }}>
              <label className="text-xs text-gray-900">Contenido</label>
              <SmallInput name="contenido" type="text" placeholder="iPhone 14 pro Max"
                value={draft.contenido} onChange={handleDraftChange} />
            </div>
          </div>
          <div className="flex justify-end mt-3">
            <button type="button" onClick={handleAdd}
              className="flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-blue-600 transition-all"
              style={{ padding: "8px 14px", borderRadius: "8px", background: "#fff", border: "1.5px solid #e5e7eb" }}>
              Agregar
        ...
        {paquetes.length > 0 && (
          <div className="flex flex-col gap-3 mb-4">
            {paquetes.map((pkg, i) => (
              <div key={i} className="flex flex-wrap items-center gap-3 rounded-xl px-4 py-3"
                style={{ border: "1.5px solid #86efac", minHeight: "72px" }}>
                <div className="flex flex-col gap-1" style={{ minWidth: "110px" }}>
                  <span className="text-xs text-gray-900">Peso en libras</span>
                  <div style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: "8px", padding: "6px 10px" }}>
                    <span className="text-sm text-gray-800">{pkg.peso} libras</span>
                  </div>
                </div>
                <div className="flex flex-col gap-1 flex-1" style={{ minWidth: "130px" }}>
                  <span className="text-xs text-gray-900">Contenido</span>
                  <div style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: "8px", padding: "6px 10px" }}>
                ...
                <div className="flex flex-col gap-1.5 flex-shrink-0">
                  <div className="grid grid-cols-3">
                    {["Largo", "Alto", "Ancho"].map((label) => (
                      <span key={label} className="text-xs font-semibold text-gray-900 px-4">{label}</span>
                    ))}
                  </div>
                  <div className="grid grid-cols-3" style={{ border: "1.5px solid #e5e7eb", borderRadius: "10px", overflow: "hidden", background: "#fff" }}>
                    {[pkg.largo, pkg.alto, pkg.ancho].map((val, idx) => (
                      <div key={idx} className="flex items-center gap-2 px-4 py-2.5"
                        style={{ borderRight: idx < 2 ? "1.5px solid #e5e7eb" : "none" }}>
                ...
                <button type="button" onClick={() => handleRemove(i)}
                  ...
                  style={{ width: "34px", height: "34px", borderRadius: "8px", border: "1.5px solid #fca5a5", background: "#fff", color: "#f87171" }}>
